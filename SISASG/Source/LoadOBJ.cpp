...
)
{
    ...
    std::vector <pair3> group; // A pair of  groups shit
    while(!fileStream.eof())
    {
        char buf[256];
        fileStream.getline(buf, 256); // Why 256? meh... what evs.
        ...
        // This is a group.
        else if (strncmp("g ", buf, 2) == 0)
        {
            group.push_back({ vertexIndices,uvIndices,normalIndices });
            vertexIndices.clear();
            uvIndices.clear();
            normalIndices.clear();
        }
    ...
    for (size_t i = 0; i < group.size(); i++)
    {
        ...
        uvIndices = group[i].b; // uvIndex
        normalIndices = group[i].c; // normalIndex
        PTVPair pair;
